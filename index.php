...
$icon_url = plugins_url('images/phpswcf.png', __FILE__);
...
?>
<h4>Fill out the given form to Contact Us</h4>
<?php
if($emailSent){
echo '<p><font color="green">Thank you for contacting us, we will reply you within 5-7 business days.</font></p>';
}
if(!empty($errors)){
echo '<p>'; foreach($errors as $error){ echo '<font color="red">'.$error.'</font><br />'; } echo '</p>';
}
?>
<form action="<?php the_permalink(); ?>" method="post">

<label for="userName">Your Name: </label><span class="required">*</span><br />
<input type="text" name="userName" id="userName" class="regular-text" size="40" 
value="<?php if(!empty($userName)){ echo esc_html($userName); } ?>" /><br />

<label for="userEmail">Your Email ID: </label><span class="required">*</span><br />
<input type="text" name="userEmail" id="userEmail" class="regular-text" size="40" 
value="<?php if(!empty($userEmail)){ echo esc_html($userEmail); } ?>" /><br />

<label for="emailSubject">Email Subject: </label><span class="required">*</span><br />
<input type="text" name="emailSubject" id="emailSubject" class="regular-text" size="40" 
value="<?php if(!empty($emailSubject)){ echo esc_html($emailSubject); } ?>" /><br />

<label for="emailMessage">Email Message: </label><span class="required">*</span><br />
<textarea name="emailMessage" id="emailMessage" class="large-text" rows="8" cols="40">
<?php if(!empty($emailMessage)){ echo esc_html($emailMessage); } ?>
</textarea><br />

<?php wp_nonce_field('PhpswCFNonceSubmitted','PhpswCFNonceField'); ?>
...
echo '<p><a href="http://69plugins.com/free-plugins/phpsword-contact-form/" target="_blank" title="PhpSword Contact Form WordPress plugin by 69plugins.com">PhpSword Contact Form</a></p>';
}
?>
